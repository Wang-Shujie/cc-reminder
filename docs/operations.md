...
- 主导航为：**工作台 / 通知规则 / 项目 / 集成 / 设置**；工作台内含「状态概览」与「通知记录」，集成内含「通知来源」与「通知去向」。
...
- **Agent 集成** 页每次检测都会比对磁盘上的 Hook 定义与应用记录的指纹，状态列为：**健康 / 缺失 / 不一致 / Helper 版本不匹配 / 待确认 / 版本未验证（需重装此条目）**。待确认条目旁会给出**触发建议**——可直接复制的提示词（权限请求 / 工具调用 / 子代理 / 普通对话）或操作指引（新开会话 / 退出会话 / 触发压缩），完成一次真实触发即转健康。
...
- 工作台 · 状态概览的指标（待发送 / 等待重试 / 失败 / 过期 / 暂存 / 被拒绝）来自与托盘、设置页共享的健康快照。
...
| `health.json` | 当时的完整健康快照（工作台 · 状态概览同一数据源） |
...
- Agent 升级到能力目录尚未验证的版本：应用仍可安装 Hook，但仅启用经目录验证的**安全事件子集**（SessionStart / SessionEnd / PermissionRequest / Stop），条目按真实状态显示；待 CC Reminder 更新能力目录后重新安装即可启用完整事件集。仅当条目因此缺失时显示**版本未验证（需重装此条目）**。
...
**P1 首次启动**：全新用户账号（或已备份/改名上表数据目录）安装并启动；确认引导向导出现且五步可完成；确认 **工作台 · 状态概览** 无异常问题项。
...
**P10 离线 spool 恢复 / TTL**：停止应用 → 用 Helper CLI 发送 N 个合法事件（部分写入 spool）→ 记录时间 → 等待超过规则 TTL 后启动应用 → 核对：TTL 内事件补发、超 TTL 事件标记 **已过期**、工作台指标 **{n} 个暂存事件** 归零。
