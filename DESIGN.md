...
- **设置页(2026-08-28 三次裁决,CC Switch 式,取代两列卡组)**:无卡片框单列分组,每组 = 标题 + 灰色说明 + 控件;启动与窗口 = 整行开关行(标题+说明+软轨开关居右);界面语言/外观主题/调试时长 = 分段胶囊(原生 radio 铺在胶囊下的透明输入层,键盘方向键与命中语义不变;设置页激活段系统蓝实心白字,规则页分段保持白卡浮起);数据保留 = 并排数字域;通知模板转同构组。整页自动保存无保存按钮(即时存 + 数字/模板防抖 600ms;scheduleSave 须显式并入 patch 否则同步调用写回旧值——实测竞态;主题"未保存预览回滚"已随自动保存删除)。开关 = 36×20 无描边软轨道;单选(抽屉/工作日)= 蓝盘白心。
